...
STAMP = """
<p class="stamp">Regenerated by <code>tools/make_screens.py</code> from commit <b>%s</b>
on %s UTC. Every map picture below is the painted one: if you still see glyphs on a
black background, you are looking at an old copy of this page (browser cache, or a
previously downloaded file) &mdash; the colours in <code>docs/shots/</code> are what the
running game draws.</p>
"""
...
def stamp():
    import time
    return STAMP % (git_short(), time.strftime("%Y-%m-%d %H:%M"))
...
def main():
 ...
 p.sub{color:#a8a8be}
 .stamp{color:#c9c9dc;background:#181826;border-left:3px solid #ffd75e;padding:10px 14px;border-radius:6px;font-size:14px}
